...
router.get('/reports/capacity', async (req, res) => {
  try {
    ...
    const totalHours = result.rows.reduce((sum, user) => sum + (parseFloat(user.total_hours) || 0), 0);
    const activeUsers = result.rows.filter(user => parseFloat(user.total_hours) > 0).length;
    ...
    res.json({
      ...
      team_summary: {
        ...
        utilization_percentage: Math.round((avgHours / 40) * 100 * 10) / 10 // Assuming 40h/week target
      },
